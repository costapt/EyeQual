...
def map_to_patch(y, x, n_blocks=3):
    overlap = 2**n_blocks
    t = lambda v: v*overlap - (overlap - 1)
    return t(y), t(x)


def get_gaussian_quality_map(heatmap, n_blocks=3, input_size=512):
    size = (input_size, input_size)
    quality_map = np.zeros(size)
    s = np.zeros(size)
    blocks_to_size = {3: 22, 4: 46, 5: 94}
    f_size = blocks_to_size[n_blocks]
    out_size = 512 / (2**n_blocks)
    f = gaussian_filter(shape=(f_size, f_size), sigma=7)
    for y in range(out_size):
        for x in range(out_size):
            yi, xi = map_to_patch(y, x, n_blocks=n_blocks)
            ...
            quality_map[yi:yf, xi:xf] += curr_f * heatmap[y, x]
